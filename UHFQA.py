...
def do_plot2d(P1_values, P2_values, sweep_data_2D, paths, readout_length, P2_name, save_fig = 0):
    ...
    unwrap = 1
    xlabel = 'temp'
    if np.floor(P1_values[0]/1e9) >= 1:
        P1_values =  P1_values/1e9
        xlabel = 'Frequency (GHz)'
    else:
    ...
    ax1.set_ylabel(y1label)
#    ax1.set_xlabel(xlabel)
    ax1.semilogy(P1_values, np.abs(data_cmpl),'-o', markersize = 3, c='C0')
#    ax1.semilogy(P1_values, np.abs(data_cmpl).transpose(),c='C0')
    ax2.set_ylabel(y2label)
    ax2.set_xlabel(xlabel)
    if unwrap:
        phase = np.unwrap(np.angle(data_cmpl))
        ...
        phase = np.angle(data_cmpl)
    print(phase.size)
    ax2.plot(P1_values, phase*180/pi, '-o', markersize = 3, c='C1')
    fig.align_ylabels([ax1,ax2])
    fig.set_tight_layout(True)
    plt.show()
#     fig.canvas.manager.window.move(0, 500)
    ...
    x_values[1:]=P2_values+(P2_values[1]-P2_values[0])/2

#    x_values=np.append(P2_values[0]/2, P2_values[1:]-(P2_values[1]-P2_values[0]/2), P2_values[-1]+(P2_values[1]-P2_values[0]/2))
    y_values=P1_values
    ...
    z_amp_max=np.max(z_amp)
    ax1.set_title('Amplitude')
    ax1.set_ylabel(xlabel)
    c1 = ax1.pcolormesh(XX,YY, z_amp, cmap='RdBu',vmin=z_amp_min, vmax=z_amp_max)
    plt.tight_layout()
    ...
    z_pha_max=np.max(z_pha)
    c2 = ax2.pcolormesh(XX,YY, z_pha, cmap='RdBu',vmin=z_pha_min, vmax=z_pha_max)
#    ax1.set_title('pcolormesh')
    ...
    plt.tight_layout()
    if save_fig == 1:
        plt.savefig("spectroscopy 2D " +time.ctime(time.time()).replace(":","-") )

def extract_freq(t_vector,y_vector,dt):
    N = len(t_vector)
...
def pulse_plot1d(sequence,dt, qubitDriveFreq , amplitude_hd,x_vector,y_vector, AC_pars=[0,0],RT_pars=[0,0], fitting = 1, save_fig = 0,iteration=1):
    ...
    if fitting == 1:
        if sequence == "rabi":
            ...
            period = 1/extract_freq(x_vector, abs_camplitude, dt)
            phase = 0
            p0 = [amp,period,phase,offset]
            best_vals, covar = scy.optimize.curve_fit(rabi, x_vector, abs_camplitude,p0=p0,xtol=1e-6,maxfev=3000)
            pi_pulse = np.round(1/2*best_vals[1]*1e3)
            ax.plot(x_vector,rabi(x_vector, best_vals[0], best_vals[1], best_vals[2],best_vals[3]),'r')
            ax.set_title('Rabi Measurement %03d'%(iteration))
            print("Pi pulse duration is %.1f ns"%(pi_pulse))
            textstr = '$\omega_d$ = %.4f GHz\n$A_d$ = %.2f\n$T_{\pi/2}$=%.1f ns\nAC $\mu$ = %.3f V'%(qubitDriveFreq*1e-9,amplitude_hd,pi_pulse,AC_pars[0])
            ...
            print('Initial Guess for Freq:%.4f'%(f))
            if f< 0.1:
                f = 0
            tau = 20
            phase = 0
            ...
            try:
                ...
                textstr = '$\omega_d$ = %.4f GHz\n$A_d$ = %.2f\n$\Delta$=%.2f MHz\n$T_2^*$=%.2f $\mu$s\n$\mu$ = %.3f V\n$\sigma$ = %.2f V\n$B_0$ = %.2f V\n$\\tau$ = %.2f $\mu s$'%(qubitDriveFreq*1e-9,amplitude_hd,best_vals[1],best_vals[3],AC_pars[0],AC_pars[1],RT_pars[0],RT_pars[1])
                plt.gcf().text(1, 0.25, textstr, fontsize=14)
