...
#     global daq_hd, device_hd
    ...
    if not use_discovery:
        ...
        daq_hd, device_hd, _ = zhinst.utils.create_api_session(device_hd_id, apilevel_example)
    ...
    if sequence == 'qubit spec':
       ...
       _add_AC_stark_
        ...
        if AC_pars[0] != 0:
            ...
            AC_stark_tone = AC_pars[0]*np.ones(qubit_drive_dur)
            ...
            awg_program = awg_program.replace('playZero(wave_dur_sample,AWG_RATE_600MHZ)','playWave(2,%f*ones(%d))'%(AC_pars[0],nPointsPre+qubit_drive_dur+nPointsPost))
        ...
        if AC_pars[0]!=0:
            qubit_drive_tone = amplitude_hd*np.ones(nPoints)
            qubit_drive_tone = qubit_drive_tone[...,None]
            AC_stark_tone = AC_pars[0]*np.ones(nPoints)
            AC_stark_tone = AC_stark_tone[...,None] # turns row vector into column vector
            wfm_2D_arr = np.hstack((qubit_drive_tone,AC_stark_tone))
            fileName = "rabi_wfm"
            np.savetxt("C:/Users/LFL/Documents/Zurich Instruments/LabOne/WebServer/awg/waves/"+fileName+".csv", wfm_2D_arr, delimiter = ",")
        ...
        awg_program = awg_program.replace('_c3_', str(amplitude_hd))
        awg_program = awg_program.replace('_c4_',str(nAverages))
        awg_program = awg_program.replace('_c5_',str(nSteps))
        ...
        awg_program2 = awg_program2.replace('_c0_', str(fs))
        awg_program2 = awg_program2.replace('_c1_',str(pulse_length_increment/2))
        awg_program2 = awg_program2.replace('_c2_', str(amplitude_hd))
        # awg_program2 = awg_program2.replace('_c3_',str(round(2*pi2Width)))
        awg_program2 = awg_program2.replace('_c3_',str(piWidth_Y))
        awg_program2 = awg_program2.replace('_c4_',str(nAverages))
        awg_program2 = awg_program2.replace('_c5_',str(nSteps))
        if fs == 2.4e9:
            awg_program2 = awg_program2.replace('AWG_RATE_1200MHZ','AWG_RATE_2400MHZ')


        if AC_pars[0] != 0 and RT_pars[0] == 0:
            fileName = "echo_wfm"
            txt = "//Make pre-pulse\nwave AC_stark_tone_pre = %f*ones(%d);\n"%(AC_pars[0],nPointsPre)+"wave pi2pulse_pre_zeros = zeros(%d);\n"%(nPointsPre-pi2Width)+"wave pi2pulse_pre=join(pi2pulse_pre_zeros,pi2pulse);\nassignWaveIndex(pi2pulse_pre,AC_stark_tone_pre,1);\n\n"
            txt += "//Make post-pulse\nwave AC_stark_tone_post = %f*ones(%d);\n"%(AC_pars[0],nPointsPost)+"wave pi2pulse_post_zeros = zeros(%d);\n"%(nPointsPost-pi2Width)+ "wave pi2pulse_post=join(pi2pulse,pi2pulse_post_zeros);\nassignWaveIndex(pi2pulse_post,AC_stark_tone_post,2);\n\n"
            txt += "//Make mid-pulse\nwave AC_stark_mid_pulse = %f*ones(2*_c4_);\n"%(AC_pars[0])+"assignWaveIndex(pipulse,AC_stark_mid_pulse,3);\n"
            txt += "\nwave wfms = \"%s\";\n"%(fileName)+"assignWaveIndex(wfms,0);\n"
            awg_program = awg_program.replace('_add_white_noise_',txt)
            txt1 = "playWave(join(pi2pulse_pre,pipulse,pi2pulse_post),join(AC_stark_tone_pre,AC_stark_mid_pulse,AC_stark_tone_post));"
            txt2 = 'executeTableEntry(_c6_);'
            txt3 = 'executeTableEntry(_c6_+1);'
            # txt4 = "playWave(pipulse,%f*ones(2*_c4_));"%(AC_pars[0])
            ...
            fileName = "echo_wfm"
            txt += "const marker_pos = %d;\n" %nPointsPre
            txt += "wave marker_left = marker(marker_pos-%d,0);\nwave marker_right = marker(%d,1);\nwave w_marker = join(marker_left,marker_right);\n"%(pipulse_position,pipulse_position)
            txt += "wave AC_stark_tone_pre_w_marker = AC_stark_tone_pre + w_marker;\n"
            txt += "wave wfms = \"%s\";\n"%(fileName)+"assignWaveIndex(wfms,0);\n"
            txt += "wave AC_stark_mid_pulse = %f*ones(%d);" %(AC_pars[0],round(2*pi2Width))
            awg_program = awg_program.replace('_add_white_noise_',txt)
            awg_program = awg_program.replace('_add_AC_pre_pulse_',"playWave(pi2pulse_pre,AC_stark_tone_pre_w_marker);")
            awg_program = awg_program.replace('_add_AC_post_pulse_',"playWave(pi2pulse_post,AC_stark_tone_post);")
            awg_program = awg_program.replace('_add_mid_pulse_','')
            awg_program = awg_program.replace('executeTableEntry(i);executeTableEntry(i);','executeTableEntry(i);')
